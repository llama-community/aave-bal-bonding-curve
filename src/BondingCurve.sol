...
import {IBondingCurve} from "./external/IBondingCurve.sol";
import {OracleRef} from "./external/OracleRef.sol";
import {PCVSplitter} from "./external/PCVSplitter.sol";
import {Incentivized} from "./external/Incentivized.sol";
import {IPCVDeposit} from "./external/IPCVDeposit.sol";
import {Timed} from "./external/Timed.sol";
...
contract BondingCurve is IBondingCurve, OracleRef, PCVSplitter, Timed, Incentivized {
    ...
    function setBuffer(uint256 newBuffer) external override onlyGovernorOrAdmin {
        require(newBuffer < BASIS_POINTS_GRANULARITY, "BondingCurve: Buffer exceeds or matches granularity");
    ...
    function setDiscount(uint256 newDiscount) external override onlyGovernorOrAdmin {
        require(newDiscount < BASIS_POINTS_GRANULARITY, "BondingCurve: Buffer exceeds or matches granularity");
    ...
    function setAllocation(address[] calldata allocations, uint256[] calldata ratios) external override onlyGovernor {
    ...
    function _allocateSingle(uint256 amount, address pcvDeposit) internal virtual override {
